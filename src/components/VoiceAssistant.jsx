...
const VoiceAssistant = () => {
    ...
    // Auto-open logic after 1.5 seconds, but wait for interaction for audio
    useEffect(() => {
        const timer = setTimeout(() => {
            if (!hasAutoPlayed) {
                setIsOpen(true);
                // Try to speak immediately (might be blocked)
                speak(hindiScript);
    ...
    // Re-trigger speak if user interacts and it hasn't played firmly? 
    // Actually, simply opening the window is enough. The 'Replay' button is the fallback.
    // We will ensure the 'speak' function handles errors gracefully.
